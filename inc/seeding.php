 ...
 *
 * Runs once per gate option on the first wp-admin visit after deploy, then
 * never again. Safe to keep deployed: it exits instantly once seeded, and it
 * refuses to run if a post with the slug already exists.
 */
defined('ABSPATH') || exit;

add_action('admin_init', 'rmd_seed_mariner_case_study');
function rmd_seed_mariner_case_study() {
	if ('1' === get_option('rmd_seed_mariner_v1')) {
		return;
	}
	...
	if (get_page_by_path('mariner-underwear', OBJECT, 'case_study')) {
		update_option('rmd_seed_mariner_v1', '1');
		return;
	}
